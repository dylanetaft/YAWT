...
def main():
    ...
    server_bin = os.path.join(examples_dir, "wt_server")
    client_bin = os.path.join(examples_dir, "wt_client")

    if not os.path.isfile(server_bin):
        print(f"FAIL: wt_server not found at {server_bin}", file=sys.stderr)
        print(f"      hint: run `make -C build` first, or set YAWT_EXAMPLES_DIR", file=sys.stderr)
        return 1

    if not os.path.isfile(client_bin):
        print(f"FAIL: wt_client not found at {client_bin}", file=sys.stderr)
        print(f"      hint: run `make -C build` first, or set YAWT_EXAMPLES_DIR", file=sys.stderr)
        return 1
    ...
    with tempfile.TemporaryDirectory(prefix="yawt_test_") as tmpdir:
        ...
        try:
            ...
            # wt_client verifies the server cert against the generated cert
            # (passed as its CA file); CN=localhost matches the SNI hostname.
            ...
            if result.returncode != 0:
                print(f"FAIL: wt_client exited {result.returncode}", file=sys.stderr)
                print(f"stdout: {result.stdout[:500]}", file=sys.stderr)
                print(f"stderr: {result.stderr[:500]}", file=sys.stderr)
                return 1

            if "Hello from wt_client!" not in result.stdout:
                print(f"FAIL: expected datagram echo, got: {result.stdout[:200]!r}", file=sys.stderr)
                return 1

            if "Stream hello from wt_client!" not in result.stdout:
                print(f"FAIL: expected stream echo, got: {result.stdout[:200]!r}", file=sys.stderr)
                return 1

            print("PASS: wt integration test")
            return 0

        except subprocess.TimeoutExpired:
            print("FAIL: wt_client timed out", file=sys.stderr)
            return 1
        finally:

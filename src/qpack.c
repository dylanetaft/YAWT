...
static YAWT_QPACK_HuffDecoder_t g_huff_decoder;
static int                      g_huff_decoder_ready = 0;

static void huff_decoder_build(void) {
    g_huff_decoder.root = 0;
    g_huff_decoder.count = 1;
    // root node already zeroed (static)

    uint32_t mask = (uint32_t)1 << 31;
    for (uint16_t i = 0; i < 256; i++) {
        uint16_t current = g_huff_decoder.root;
        for (uint8_t b = 0; b < HUFFMAN_TABLE[i].bits; b++) {
            ...
            if (!bit) {
                if (g_huff_decoder.nodes[current].l == 0) {
                    g_huff_decoder.nodes[current].l = g_huff_decoder.count;
                    g_huff_decoder.count++;
                }
                current = g_huff_decoder.nodes[current].l;
            } else {
                if (g_huff_decoder.nodes[current].r == 0) {
                    g_huff_decoder.nodes[current].r = g_huff_decoder.count;
                    g_huff_decoder.count++;
                }
                current = g_huff_decoder.nodes[current].r;
            }
        }
        g_huff_decoder.nodes[current].value = (uint8_t)i;
        g_huff_decoder.nodes[current].bits = HUFFMAN_TABLE[i].bits;
    }
    g_huff_decoder_ready = 1;
}
    ...
    size_t *bit_offset, uint8_t *out_byte)
{
    ...
    while (1) {
        ...
        if (g_huff_decoder.nodes[current].l == 0 &&
            g_huff_decoder.nodes[current].r == 0) {
            *out_byte = g_huff_decoder.nodes[current].value;

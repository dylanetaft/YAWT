...
static void _preemptive_fc_stream_rx_limit_check(YAWT_Q_Context_t *con, YAWT_Q_StreamUserData_t *sud)
{
  ...
  if (SUD_META(sud)->fc.rx_max_data > 0 && pct > 0 && pct <= 100) {
    ...
    if (SUD_META(sud)->rx_highest_offset >= threshold) {
      YAWT_Q_FlowControlInfo_t info = {
        ...
        .consumed = SUD_META(sud)->rx_highest_offset
      };
      YAWT_Q_EventParam_t param;
...
static void _preemptive_fc_stream_tx_limit_check(YAWT_Q_Context_t *con, YAWT_Q_StreamUserData_t *sud)
{
  ...
  if (SUD_META(sud)->fc.tx_max_data > 0 && pct > 0 && pct <= 100) {
    ...
    if (SUD_META(sud)->tx_next_offset >= threshold) {
      YAWT_Q_FlowControlInfo_t info = {
        .type = YAWT_Q_FC_STREAM_TX,
        .stream_id = SUD_META(sud)->stream_id,
        .current_limit = SUD_META(sud)->fc.tx_max_data,
        .consumed = SUD_META(sud)->tx_next_offset
      };
      YAWT_Q_EventParam_t param;
      param.P_EVT_FLOW_CONTROL.info = &info;
      param.P_EVT_FLOW_CONTROL.stream_ud = sud;
...
YAWT_Q_Context_t *YAWT_q_con_create(YAWT_Q_Con_Create_Info_t *info) {
  ...
  /* RFC 9000 §2.1: first local stream ID equals the type's low-2-bit value */
  if (con->role == YAWT_Q_ROLE_SERVER) {
    con->next_local_stream_id_bidi = YAWT_Q_S_BIDI;
    con->next_local_stream_id_uni  = YAWT_Q_S_UNI;
  } else {
    con->next_local_stream_id_bidi = YAWT_Q_C_BIDI;
    con->next_local_stream_id_uni  = YAWT_Q_C_UNI;
  }
...
static YAWT_Q_StreamUserData_t *_stream_meta_add(YAWT_Q_Context_t *con, uint64_t stream_id) {
  ...
  /* Advance local stream-ID high-water so hardcoded openers (H3 control/QPACK)
   * and next_stream_id stay coherent. next_stream_id may already have reserved
   * this ID (stream_id < next); only bump when this create is at or past next. */
  if (we_initiated) {
    if (is_bidi) {
      if (stream_id >= con->next_local_stream_id_bidi)
        con->next_local_stream_id_bidi = stream_id + 4;
    } else {
      if (stream_id >= con->next_local_stream_id_uni)
        con->next_local_stream_id_uni = stream_id + 4;
    }
  }
...
uint64_t YAWT_q_con_next_stream_id(YAWT_Q_Context_t *con, bool is_bidi) {
  if (!con) return 0;
  // RFC 9000 §2.1: per-role, per-direction IDs step by 4. Counters are seeded
  // in con_create and kept ahead of any local stream via _stream_meta_add
  // (including H3's hardcoded control/QPACK IDs).
  uint64_t *next = is_bidi ? &con->next_local_stream_id_bidi
                           : &con->next_local_stream_id_uni;
  uint64_t id = *next;
  *next = id + 4;
  return id;
}
  ...
  while (frc.cursor < frc.len && frc.err == YAWT_Q_OK) {
    ...
    switch (frame.type) {
      ...
      case YAWT_Q_FRAME_STREAM: {
        ...
        if (SUD_META(sud)->rx_highest_offset > SUD_META(sud)->fc.rx_max_data) {
          YAWT_LOG(YAWT_LOG_ERROR, "FLOW_CONTROL_ERROR: stream %lu exceeded limit (%lu > %lu)",
                   SUD_META(sud)->stream_id, SUD_META(sud)->rx_highest_offset, SUD_META(sud)->fc.rx_max_data);
      ...
      case YAWT_Q_FRAME_STREAM_DATA_BLOCKED: {
        ...
        uint64_t consumed = sud ? SUD_META(sud)->tx_next_offset : 0;
        YAWT_Q_FlowControlInfo_t info = {
          .type = YAWT_Q_FC_STREAM_TX,
    ...
    while (1) {
      ...
      while ((item = ANB_slab_peek_item_iter(con->tx_buffer, &iter, &item_size)) != NULL) {
        ...
        if (f->type == YAWT_Q_FRAME_STREAM) {
          ...
          if (sud && SUD_META(sud)->tx_next_offset > SUD_META(sud)->fc.tx_max_data) {
      ...
      while ((item = ANB_slab_peek_item_iter(con->tx_buffer, &iter2, &item_size)) != NULL) {
        ...
        if (f->type == YAWT_Q_FRAME_STREAM) {
          ...
          YAWT_Q_StreamUserData_t *sud = _stream_meta_find(con->stream_userdata, f->stream_id);
          if (sud && SUD_META(sud)->tx_next_offset > SUD_META(sud)->fc.tx_max_data) continue;
        }
